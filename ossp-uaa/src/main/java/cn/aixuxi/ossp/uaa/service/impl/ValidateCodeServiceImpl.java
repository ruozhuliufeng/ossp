...
public class ValidateCodeServiceImpl implements IValidateCodeService {
    ...
    /**
     * 发送短信验证码<br>
     * 1.先去redis查询60s内是否已经发送验证码<br>
     * 2.未发送：判断手机号是否存在？false:产生4位数字 手机号-验证码<br>
     * 3.发往消息中心=>发送消息<br>
     * 4.保存redis
     *
     * @param mobile 手机号
    ...
    public Result sendSmsCode(String mobile) {
        ...
        if (user == null){
            log.error("用户用户手机号{}查询用户为空",mobile);
            return Result.failed("手机号不存在");
        }
        // 产生随机4位数字
        String code = RandomUtil.randomNumbers(4);
        log.info("短信发往请求消息中心 ==> 手机号：{} ==> 验证码：{}",mobile,code);
