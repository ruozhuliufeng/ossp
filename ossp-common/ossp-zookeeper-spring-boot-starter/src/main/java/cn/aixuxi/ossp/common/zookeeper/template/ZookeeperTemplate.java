package cn.aixuxi.ossp.common.zookeeper.template;
 ...
 * @date 2021-08-31
 *  注解注释：@SneakyThrows
 *      是lombok包下的注解，并且集成了Throwable，作用是为了try{}catch{}捕捉异常
 *      添加之后会在代码编译时自动捕获异常
 */
@Component
public class ZookeeperTemplate {
     ...
     */
    @SneakyThrows
    private String createNode(String path, String node, CreateMode createMode) {
    ...
    public String update(String path,String node,String value){
        Assert.isTrue(StrUtil.isNotEmpty(value),
                "Zookeeper节点值不能为空！");
        path = buildPath(path,node);
    ...
    /**
     * 对一个节点进行监听，监听事件包括对指定路径节点的增、删、改的操作
     * @param path 节点路径
     * @param listener 回调方法
     */
    ...
    /**
     * 对指定的路径节点的一级子目录进行监听，不对该节点的操作进行监听，对齐子目录的节点进行增、删、改的操作监听
     * @param path 节点路径
     * @param listener 回调方法
     */
    ...
    /**
     * 将指定的路径节点作为跟节点(祖先节点)，对其所有的子节点操作进行监听，呈现树形目录的监听
     * @param path 节点路径
     * @param maxDepth 回调方法
     * @param listener 监听器
     */
