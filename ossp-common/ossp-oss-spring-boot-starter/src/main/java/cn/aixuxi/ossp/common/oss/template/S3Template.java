...
public class S3Template implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        AwsClientBuilder.EndpointConfiguration endpoint = new AwsClientBuilder.EndpointConfiguration(properties.getS3().getEndpoint(), properties.getS3().getRegion());
        AWSCredentials credentials = new BasicAWSCredentials(properties.getS3().getAccessKey(), properties.getS3().getSecretKey());
        AWSCredentialsProvider provider = new AWSStaticCredentialsProvider(credentials);
        this.amazonS3 = AmazonS3Client.builder()
                .withEndpointConfiguration(endpoint)
                .withClientConfiguration(config)
                .withCredentials(provider)
                .withPathStyleAccessEnabled(properties.getS3().getPathStyleAccessEnabled())
    ...
    public ObjectInfo upload(String fileName, InputStream is) {
        return upload(properties.getS3().getBucketName(), fileName, is, is.available(), DEF_CONTEXT_TYPE);
    }

    @SneakyThrows
    public ObjectInfo upload(MultipartFile file) {
        return upload(properties.getS3().getBucketName(), file.getOriginalFilename(), file.getInputStream(),
     ...
     * @return ObjectInfo
     */
    private ObjectInfo upload(String bucketName, String fileName, InputStream is, int size, String contentType) {
        ...
        obj.setObjectPath(bucketName + PATH_SPLIT + fileName);
        obj.setObjecUrl(properties.getS3().getEndpoint() + PATH_SPLIT + obj.getObjectPath());
        return obj;
    }
    ...
    public void out(String objectName, OutputStream os) {
        out(properties.getS3().getBucketName(), objectName, os);
    }
